...
let lst = []; let fav = []; let myFoods = []; let hist = []; let bodyData = []; let chatHistory = []; let selIdx = -1; let selMyIdx = -1; let editIdx = -1;
let deletedToday = { date: "", items: [] };
let favoriteSortMode = false;
let favoriteSettings = {};
let favoriteQuickPanelMode = false;
...
function normalizeMyFoods() {
    const now = Date.now();
    myFoods = (Array.isArray(myFoods) ? myFoods : []).map((x, i) => ({
        N: x.N || x.n || "不明",
        P: parseNum(x.P !== undefined ? x.P : x.p),
        F: parseNum(x.F !== undefined ? x.F : x.f),
        C: parseNum(x.C !== undefined ? x.C : x.c),
        A: parseNum(x.A !== undefined ? x.A : x.a),
        Cal: Math.round(parseNum(x.Cal !== undefined ? x.Cal : x.cal)),
        Fav: !!(x.Fav || x.favorite || x.isFavorite),
        favoriteOrder: Number.isFinite(Number(x.favoriteOrder)) ? Number(x.favoriteOrder) : i,
        ...
        createdAt: Number.isFinite(Number(x.createdAt)) ? Number(x.createdAt) : now + i
    }));
}
...
function loadFavoriteSettings() {
    try {
        favoriteSettings = JSON.parse(localStorage.getItem('tf_fav_settings') || "{}");
    } catch (e) {
        favoriteSettings = {};
    }
    if (!favoriteSettings || typeof favoriteSettings !== 'object') favoriteSettings = {};
}

function saveFavoriteSettings() {
    localStorage.setItem('tf_fav_settings', JSON.stringify(favoriteSettings));
}

function favoriteKey(source, i) {
    return `${source}:${i}`;
}

function getFavoriteSetting(source, i) {
    const key = favoriteKey(source, i);
    if (!favoriteSettings[key]) favoriteSettings[key] = {};
    return favoriteSettings[key];
}

function removeFavoriteSetting(source, i) {
    delete favoriteSettings[favoriteKey(source, i)];
    saveFavoriteSettings();
}

function removeMyFoodFavoriteSettings(deletedIndex) {
    const nextSettings = {};
    Object.entries(favoriteSettings || {}).forEach(([key, value]) => {
        const match = key.match(/^my:(\d+)$/);
        if (!match) {
            nextSettings[key] = value;
            return;
        }
        const index = Number(match[1]);
        if (index < deletedIndex) nextSettings[key] = value;
        if (index > deletedIndex) nextSettings[favoriteKey('my', index - 1)] = value;
    });
    favoriteSettings = nextSettings;
    saveFavoriteSettings();
}

function getDbDefaultAmount(i) {
    const d = DB[i];
    if (!d) return 1;
    const setting = getFavoriteSetting('db', i);
    if (Number.isFinite(Number(setting.amount)) && Number(setting.amount) > 0) return Number(setting.amount);
    if (d[1].includes("白米") || d[1].includes("玄米") || d[1].includes("オート")) return 150;
    if (String(d[3] || "").includes('g')) return 100;
    return 1;
}

function getMyDefaultAmount(i) {
    const setting = getFavoriteSetting('my', i);
    if (Number.isFinite(Number(setting.amount)) && Number(setting.amount) > 0) return Number(setting.amount);
    return 1;
}

function getFavoriteUnit(item) {
    if (item.source === 'db') {
        const d = DB[item.i];
        if (!d) return "";
        if (String(d[3] || "").includes('g')) return "g";
        if (String(d[3] || "").includes('杯')) return "杯";
        return "個";
    }
    return "個";
}

function getDisplayAmount(item, amount) {
    return Number.isInteger(Number(amount)) ? String(Number(amount)) : String(Number(amount));
}

function getFavoriteAmount(item) {
    return item.source === 'db' ? getDbDefaultAmount(item.i) : getMyDefaultAmount(item.i);
}

function formatFavoriteAmount(item) {
    const amount = getFavoriteAmount(item);
    const unit = getFavoriteUnit(item);
    return `${getDisplayAmount(item, amount)}${unit}`;
}

function getFavoriteOrder(source, i, fallback) {
    const setting = getFavoriteSetting(source, i);
    return Number.isFinite(Number(setting.order)) ? Number(setting.order) : fallback;
}

function getAllFavoriteItems() {
    normalizeMyFoods();
    const dbItems = (typeof DB === 'undefined' ? [] : fav.map(i => ({ ...DB[i], name: DB[i]?.[1] || "不明", isMy: false, source: 'db', i })));
    const myItems = myFoods.map((x, i) => ({ ...x, name: x.N, isMy: true, source: 'my', i })).filter(x => x.Fav);
    return [...myItems, ...dbItems]
        .filter(x => x && x.name)
        .sort((a, b) => {
            const ao = getFavoriteOrder(a.source, a.i, a.source === 'my' ? (Number(a.favoriteOrder) || 0) : 10000 + a.i);
            const bo = getFavoriteOrder(b.source, b.i, b.source === 'my' ? (Number(b.favoriteOrder) || 0) : 10000 + b.i);
            if (ao !== bo) return ao - bo;
            return (b.lastUsedAt || 0) - (a.lastUsedAt || 0);
        });
}

function editFavoriteAmount(source, i) {
    const item = source === 'db'
        ? { source, i, name: DB[i]?.[1] || "不明", isMy: false }
        : { source, i, name: myFoods[i]?.N || "不明", isMy: true };
    const current = getFavoriteAmount(item);
    const unit = getFavoriteUnit(item);
    const answer = prompt(`「${item.name}」のいつもの量を入力してください（${unit || "個"}）`, current);
    if (answer === null) return;
    const amount = parseNum(answer);
    if (!amount || amount <= 0) return alert("1以上の数字を入力してください");
    const setting = getFavoriteSetting(source, i);
    setting.amount = amount;
    saveFavoriteSettings();
    const list = document.getElementById('f-list');
    if (list) list.dataset.cat = "";
    shwList('⭐', document.querySelector('.fav-cat-btn'));
}

function buildFavoriteLogItem(item, amount) {
    const time = getAutoTime();
    if (item.source === 'db') {
        const d = DB[item.i];
        if (!d) return null;
        let m = 1;
        if (String(d[3] || "").includes('g')) m = amount / parseFloat(d[3]);
        else m = amount;
        const P = Number(d[4] || 0) * m;
        const F = Number(d[5] || 0) * m;
        const C = Number(d[6] || 0) * m;
        const unitPfcCal = (Number(d[4] || 0) * 4) + (Number(d[5] || 0) * 9) + (Number(d[6] || 0) * 4);
        const unitA = Number.isFinite(Number(d[8]))
            ? Number(d[8])
            : ((String(d[0] || "").includes("酒") || Number(d[7] || 0) > unitPfcCal + 10) ? Math.max(0, (Number(d[7] || 0) - unitPfcCal) / 7) : 0);
        const A = unitA * m;
        const Cal = Math.round((P * 4) + (F * 9) + (C * 4) + (A * 7));
        const unit = getFavoriteUnit(item);
        return { id: Date.now(), N: `${d[1]}(${getDisplayAmount(item, amount)}${unit})`, P, F, C, A, Cal, U: d[3], time };
    }
    const d = myFoods[item.i];
    if (!d) return null;
    const P = Number(d.P || 0) * amount;
    const F = Number(d.F || 0) * amount;
    const C = Number(d.C || 0) * amount;
    const A = Number(d.A || 0) * amount;
    const Cal = Math.round(Number(d.Cal || 0) * amount);
    return { id: Date.now(), N: `${d.N}(${getDisplayAmount(item, amount)}個)`, P, F, C, A, Cal, U: "-", time };
}

function addFavoriteQuick(source, i) {
    const item = source === 'db'
        ? { source, i, name: DB[i]?.[1] || "不明", isMy: false }
        : { source, i, name: myFoods[i]?.N || "不明", isMy: true };
    const amount = getFavoriteAmount(item);
    ...
    lst.push(logItem);
    if (source === 'my' && myFoods[i]) {
        myFoods[i].useCount = (Number(myFoods[i].useCount) || 0) + 1;
        myFoods[i].lastUsedAt = Date.now();
        saveMyFoods();
    }
    sv();
    ren();
    upd();
    if (typeof showToast === 'function') showToast(`${logItem.N}を追加しました`);
}

function getFavoriteMyFoodItems() {
    normalizeMyFoods();
    return myFoods
        .map((x, i) => ({ ...x, name: x.N, isMy: true, i }))
        .filter(x => x.Fav)
        .sort((a, b) => {
            const ao = Number.isFinite(Number(a.favoriteOrder)) ? Number(a.favoriteOrder) : 9999;
            const bo = Number.isFinite(Number(b.favoriteOrder)) ? Number(b.favoriteOrder) : 9999;
            if (ao !== bo) return ao - bo;
            return (b.lastUsedAt || 0) - (a.lastUsedAt || 0);
        });
}

function reindexFavoriteOrder() {
    getAllFavoriteItems().forEach((item, order) => {
        const setting = getFavoriteSetting(item.source, item.i);
        setting.order = order;
        if (item.source === 'my' && myFoods[item.i]) myFoods[item.i].favoriteOrder = order;
    });
    saveFavoriteSettings();
    saveMyFoods();
}

function toggleMyFoodFavorite(i, btn) {
    normalizeMyFoods();
    if (!myFoods[i]) return;
    myFoods[i].Fav = !myFoods[i].Fav;
    if (myFoods[i].Fav && !Number.isFinite(Number(myFoods[i].favoriteOrder))) {
        myFoods[i].favoriteOrder = getAllFavoriteItems().length;
        getFavoriteSetting('my', i).order = myFoods[i].favoriteOrder;
    }
    if (!myFoods[i].Fav) {
        removeFavoriteSetting('my', i);
    }
    reindexFavoriteOrder();
    if (btn) {
        btn.classList.toggle('act', myFoods[i].Fav);
        btn.textContent = myFoods[i].Fav ? '★' : '☆';
    }
    const activeCat = document.getElementById('f-list')?.dataset.cat;
    if (activeCat === '⭐' || activeCat === '📂') {
        const activeBtn = activeCat === '⭐' ? document.querySelector('.fav-cat-btn') : document.querySelector('.my-cat-btn');
        const list = document.getElementById('f-list');
        if (list) list.dataset.cat = "";
        if (activeBtn) shwList(activeCat, activeBtn);
    }
}

function moveFavoriteItem(source, i, dir) {
    const favItems = getAllFavoriteItems();
    const pos = favItems.findIndex(x => x.source === source && x.i === i);
    const next = pos + dir;
    if (pos < 0 || next < 0 || next >= favItems.length) return;
    [favItems[pos], favItems[next]] = [favItems[next], favItems[pos]];
    favItems.forEach((item, order) => {
        getFavoriteSetting(item.source, item.i).order = order;
        if (item.source === 'my' && myFoods[item.i]) myFoods[item.i].favoriteOrder = order;
    });
    saveFavoriteSettings();
    saveMyFoods();
...
function moveFavoriteMyFood(i, dir) {
    moveFavoriteItem('my', i, dir);
}
...
function saveLogItemToMyFood(id, asFavorite = true) {
    const item = lst.find(x => Number(x.id) === Number(id));
    if (!item) return;
    normalizeMyFoods();
    const cleanName = String(item.N || "").replace(/^🤖\s*/, "").trim();
    const existing = myFoods.find(x => x.N === cleanName);
    if (existing) {
        existing.P = Number(item.P || 0);
        existing.F = Number(item.F || 0);
        existing.C = Number(item.C || 0);
        existing.A = Number(item.A || 0);
        existing.Cal = Math.round(Number(item.Cal || 0));
        existing.Fav = asFavorite ? true : !!existing.Fav;
        existing.lastUsedAt = Date.now();
    } else {
        const nextFavoriteOrder = getFavoriteMyFoodItems().length;
        myFoods.push({
            N: cleanName,
            P: Number(item.P || 0),
            F: Number(item.F || 0),
    ...
    reindexFavoriteOrder();
    if (typeof showToast === 'function') showToast(asFavorite ? "My食品とお気に入りに保存しました" : "My食品に保存しました");
}
...
window.onload = () => {
    ...
    if (localStorage.getItem('tf_my')) { myFoods = JSON.parse(localStorage.getItem('tf_my')); normalizeMyFoods(); saveMyFoods(); }
...
function mkCat() {
    ...
    d.innerHTML = `<div class="c-btn fav-cat-btn" onclick="shwList('⭐',this)">⭐ お気に入り入力</div><div class="c-btn my-cat-btn" onclick="shwList('📂',this)">📂 My食品</div>`;
...
function shwList(c, btn) {
    ...
    l.innerHTML = `<div class="list-head"><span>${c === '⭐' ? (favoriteQuickPanelMode ? 'クイック入力' : 'お気に入り入力') : (c === '📂' ? 'My食品' : c)}</span>${c === '⭐' && !favoriteQuickPanelMode ? `<button type="button" class="fav-sort-toggle" onclick="event.stopPropagation();toggleFavoriteSortMode()">${favoriteSortMode ? '完了' : '並び替え'}</button>` : ''}<span class="cls-btn" onclick="clsList()">× 閉じる</span></div>`;
    let itms = [];
    if (c === '📂') { if (myFoods.length === 0) l.innerHTML += `<div style="padding:15px;text-align:center;color:#666;">My食品はまだありません。</div>`; else itms = myFoods.map((x, i) => ({ ...x, name: x.N, isMy: true, source: 'my', i: i })); }
    else { const allItems = DB.map((x, i) => ({ ...x, name: x[1], isMy: false, source: 'db', i: i })); if (c === '⭐') itms = getAllFavoriteItems(); else { itms = allItems.filter(x => x[0] === c); itms.sort((a, b) => (fav.includes(b.i) ? 1 : 0) - (fav.includes(a.i) ? 1 : 0)); } }
    if (c === '⭐' && itms.length === 0) {
        l.innerHTML += favoriteQuickPanelMode
            ? `<div class="favorite-empty-guide">
                <strong>クイック入力の準備をしましょう</strong>
                <p>タップ入力で食品をお気に入りにすると、ここにカードで表示されます。</p>
                <ol>
                    <li>「タップ入力」を開く</li>
                    <li>食品の★を押してお気に入り登録</li>
                    <li>クイックカードの鉛筆でいつもの量を設定</li>
                </ol>
                <span>次からはカードを押すだけで記録できます。</span>
            </div>`
            : `<div style="padding:15px;text-align:center;color:#666;">お気に入りはまだありません。</div>`;
    }
    itms.forEach(x => {
        const d = document.createElement('div'); d.className = 'f-btn'; if (x.isMy) d.classList.add('my-food-row');
        if (c === '⭐') {
            if (favoriteQuickPanelMode && !favoriteSortMode) {
                d.classList.add('favorite-quick-row');
                d.innerHTML = `<span class="favorite-chip-main"><strong>${x.name}</strong><em>${formatFavoriteAmount(x)}</em></span><button type="button" class="quick-card-edit" aria-label="いつもの量を編集">✎</button>`;
                d.onclick = () => addFavoriteQuick(x.source, x.i);
                const editBtn = d.querySelector('.quick-card-edit');
                if (editBtn) editBtn.onclick = (e) => { e.stopPropagation(); editFavoriteAmount(x.source, x.i); };
                l.appendChild(d);
                return;
            }
        ...
        if (favoriteSortMode && c === '⭐') {
            const sortBox = document.createElement('span');
            sortBox.className = 'fav-sort-actions';
            sortBox.innerHTML = `<button type="button" onclick="event.stopPropagation();moveFavoriteItem('${x.source}',${x.i},-1)">↑</button><button type="button" onclick="event.stopPropagation();moveFavoriteItem('${x.source}',${x.i},1)">↓</button>`;
            d.appendChild(sortBox);
        } else {
            const actBtn = document.createElement('span');
            if (x.isMy) {
                actBtn.className = 'fav-icon ' + (x.Fav ? 'act' : '');
                actBtn.textContent = x.Fav ? '★' : '☆';
                actBtn.onclick = (e) => { e.stopPropagation(); toggleMyFoodFavorite(x.i, actBtn); };
                d.appendChild(actBtn);
                if (c === '📂') {
                    const delBtn = document.createElement('span');
                    delBtn.className = 'del-icon';
                    delBtn.textContent = '削除';
                    delBtn.onclick = (e) => { e.stopPropagation(); delMyFood(x.i); };
...
function setFavoriteQuickPanelMode(enabled) {
    favoriteQuickPanelMode = !!enabled;
    const panel = document.getElementById('manual-inp-sec');
    if (panel) panel.classList.toggle('quick-favorite-mode', favoriteQuickPanelMode);
}
...
function positionFoodListOverlay(anchorBtn) {
    ...
    if (favoriteQuickPanelMode) {
        l.style.position = 'static';
        l.style.left = '';
        l.style.right = '';
        l.style.top = '';
        l.style.maxHeight = 'min(52vh, 390px)';
        return;
    }
...
function clsList(shouldReturnToCategories = true) {
    const shouldCloseQuickPanel = favoriteQuickPanelMode;
    const l = document.getElementById('f-list');
    if (l) {
...
function selFd(i) {
    ...
    selIdx = i; selMyIdx = -1; editIdx = -1; document.getElementById('btn-reg').textContent = "リストに追加する"; clsList(false); showFloatingAmountPanel(amountTop);
...
function selMyFd(i) {
    const amountTop = getFloatingPanelTop();
    selIdx = -1; selMyIdx = i; editIdx = -1; document.getElementById('btn-reg').textContent = "リストに追加する"; clsList(false); showFloatingAmountPanel(amountTop);
...
function regMyFood() {
    ...
    const existing = myFoods.find(x => x.N === n);
    if (existing) {
        Object.assign(existing, data, {
            Fav: !!existing.Fav,
            favoriteOrder: Number.isFinite(Number(existing.favoriteOrder)) ? existing.favoriteOrder : getFavoriteMyFoodItems().length,
            lastUsedAt: Date.now()
        });
    } else {
        const nextFavoriteOrder = getFavoriteMyFoodItems().length;
    ...
    saveMyFoods();
    const list = document.getElementById('f-list');
    const activeCat = list?.dataset.cat;
    if (activeCat === '⭐' || activeCat === '📂') {
        list.dataset.cat = "";
        shwList(activeCat, document.querySelector(activeCat === '⭐' ? '.fav-cat-btn' : '.my-cat-btn'));
    }
    if (typeof showToast === 'function') showToast(`「${n}」をMy食品に登録しました`);
    else alert(`「${n}」をMy食品に登録しました！`);
}

function delMyFood(i) { if (!confirm(`「${myFoods[i].N}」を削除しますか？`)) return; myFoods.splice(i, 1); removeMyFoodFavoriteSettings(i); saveMyFoods(); reindexFavoriteOrder(); const list = document.getElementById('f-list'); if (list) list.dataset.cat = ""; shwList('📂', document.querySelector('.my-cat-btn')); }
...
function addM() {
    ...
    if (!wasEditing && selMyIdx >= 0 && myFoods[selMyIdx]) {
        myFoods[selMyIdx].useCount = (Number(myFoods[selMyIdx].useCount) || 0) + 1;
        myFoods[selMyIdx].lastUsedAt = Date.now();
        saveMyFoods();
    }
    selMyIdx = -1;
...
function togFav(i, el) {
    ...
    if (x >= 0) {
        fav.splice(x, 1);
        removeFavoriteSetting('db', i);
    } else {
        fav.push(i);
        getFavoriteSetting('db', i).order = getAllFavoriteItems().length;
        saveFavoriteSettings();
    }
    localStorage.setItem('tf_fav', JSON.stringify(fav));
    if (el) el.classList.toggle('act');
    const list = document.getElementById('f-list');
    const activeCat = list?.dataset.cat;
    if (activeCat === '⭐') {
        list.dataset.cat = "";
...
function importData(input) {
    ...
    reader.onload = function (e) {
        try {
            ...
            if (data.my) { let rawMy = JSON.parse(data.my); let fixedMy = rawMy.map((x, i) => ({ N: x.N || x.n || "不明", P: safeNum(x.P !== undefined ? x.P : x.p), F: safeNum(x.F !== undefined ? x.F : x.f), C: safeNum(x.C !== undefined ? x.C : x.c), A: safeNum(x.A !== undefined ? x.A : x.a), Cal: Math.round(safeNum(x.Cal !== undefined ? x.Cal : x.cal)), Fav: !!(x.Fav || x.favorite || x.isFavorite), favoriteOrder: Number.isFinite(Number(x.favoriteOrder)) ? Number(x.favoriteOrder) : i, useCount: Number.isFinite(Number(x.useCount)) ? Number(x.useCount) : 0, lastUsedAt: Number.isFinite(Number(x.lastUsedAt)) ? Number(x.lastUsedAt) : 0, createdAt: Number.isFinite(Number(x.createdAt)) ? Number(x.createdAt) : Date.now() + i })); localStorage.setItem('tf_my', JSON.stringify(fixedMy)); }
            if (data.favSettings) localStorage.setItem('tf_fav_settings', data.favSettings);
...
function exportData() {
    const data = { dat: localStorage.getItem('tf_dat'), tg: localStorage.getItem('tf_tg'), fav: localStorage.getItem('tf_fav'), favSettings: localStorage.getItem('tf_fav_settings'), my: localStorage.getItem('tf_my'), hist: localStorage.getItem('tf_hist'), date: localStorage.getItem('tf_date'), body: localStorage.getItem('tf_body') };
