export const LIVE_GUARD_VERSION='v1.7.25';
...
export function buildOfficialResolvedMessage({item,ready=false}={}){
  ...
  return `__PFC_OFFICIAL_LOOKUP_RESOLVED__ ref=${ref}. Tavily公式検索 + Gemini 3.1 Flash Lite抽出 + 公式ページ検証で栄養情報を確認し、アプリが同じ値をDraftカードへ反映済みです。source=${String(ev.sourceLabel||'公式情報')} serving=${String(ev.servingLabel||'')} kcal=${Number(ev.kcal)||0} P=${Number(ev.p)||0} F=${Number(ev.f)||0} C=${Number(ev.c)||0}。この値を唯一の正本として扱い、変更・補完・推測し直さないでください。${next}`;
}
