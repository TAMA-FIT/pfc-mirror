...
export function buildOfficialResolvedMessage({item}={}){
  ...
  return `__PFC_OFFICIAL_LOOKUP_RESOLVED__ ref=${ref}. Gemini 2.5 Flash + Google Searchで公式栄養情報を確認し、アプリが同じ値をDraftカードへ反映済みです。source=${String(ev.sourceLabel||'公式情報')} serving=${String(ev.servingLabel||'')} kcal=${Number(ev.kcal)||0} P=${Number(ev.p)||0} F=${Number(ev.f)||0} C=${Number(ev.c)||0}。この値を唯一の正本として扱い、変更・補完・推測し直さないでください。Draftがready=trueなら「画面の内容で合っていれば登録ボタンを押してください。」と短く案内してください。`;
}
