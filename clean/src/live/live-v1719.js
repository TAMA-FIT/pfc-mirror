...
import { LIVE_VERSION } from './config-v1719.js?v=1.7.19';
import { LiveMealDraft } from './draft-v1716.js?v=1.7.16';
import { GeminiLiveTransport } from './transport-v1719.js?v=1.7.19';
...
import { normalizeProvisionalFoodName, shouldRecoverTurn, buildInternalRecoveryMessage, evidenceMacroLine } from './live-guard-v1719.js?v=1.7.19';
...
function addTrace(text,{renderNow=LIVE_DEBUG}={}){const entry=`${traceStamp()} ${text}`;traceEvents.push(entry);if(traceEvents.length>8)traceEvents.shift();if(renderNow)render()}
...
function loadCss(){
  if(document.getElementById('pfc-live-v1719-css'))return;
  const link=document.createElement('link');
  link.id='pfc-live-v1719-css';link.rel='stylesheet';
  link.href=new URL('../../assets/live-v170.css?v=1.7.19',import.meta.url).href;
...
function itemStatus(item){
  ...
  if(item.unresolved)return item.candidateNames?.length?'Food Master候補を確認中':'Food Master未登録・AI推定を準備中';
...
function itemNutritionLine(item){
  const evidenceLine=evidenceMacroLine(item);
  if(evidenceLine)return evidenceLine;
  if(chooseNutritionMode(item).mode!=='trusted-db')return '';
  try{
    const r=buildRecord({foodId:item.foodId,amount:item.amount,unit:item.unit,meal:item.meal});
    if(!r)return '';
...
function render(){
  ensureModal();
  const sheet=document.getElementById('pfc-live-sheet');if(!sheet)return;
...
function scheduleTurnRecovery(delay=TURN_RECOVERY_DELAY_MS){
  if(recoveryTimer)clearTimeout(recoveryTimer);
  recoveryTimer=setTimeout(()=>{recoveryTimer=null;recoverCurrentTurn()},delay);
}
function applyLocalProvisionalCard(userText){
  ...
  try{
    draft.applyFunctionCall({id:`local-provisional-${userTurnId}`,args:{operations:[{op:'add',name}]}});
    addTrace(`local-card ${name}`);render();return true;
...
async function handleToolCalls(calls){
  ...
  for(const call of calls){
    if(call?.name!=='update_meal_draft'){
      responses.push({id:call?.id,name:call?.name||'unknown',response:{result:{ok:false,error:'unsupported_tool'}}});continue;
    }
  ...
  render();transport?.sendToolResponses(responses);
}
...
function acceptInputTranscript(text){
  if(transcriptSpeaker!=='user'){
    lastUser='';lastModel='';transcriptSpeaker='user';userTurnId+=1;resetTurnGuard();
  }
...
function armLiveHistory(){
  if(liveHistoryArmed)return;
  try{history.pushState({...((history.state&&typeof history.state==='object')?history.state:{}),__pfcLiveModal:true},'',location.href);liveHistoryArmed=true;addTrace('history-armed',{renderNow:false})}catch{}
}
async function cleanupLiveSession({clearDraft=true,hide=true}={}){
  ...
  cleanupInFlight=(async()=>{
    ...
    const tr=transport;transport=null;
    const a=audio;audio=null;
    try{t?.close()}catch{}
    try{tr?.close()}catch{}
    try{a?.stopCapture?.()}catch{}
    try{await a?.close?.()}catch{}
    if(clearDraft)draft.clear();
    sessionState='closed';
...
async function endLive(){
  pendingExplicitEnd=true;
  await cleanupLiveSession();
  if(liveHistoryArmed){
    liveHistoryArmed=false;
    try{history.back();setTimeout(()=>{if(pendingExplicitEnd)location.reload()},500);return}catch{}
  }
  ...
  try{
    ...
    transport=new GeminiLiveTransport({
      onState:s=>{
        ...
        if(s==='turn-complete'){
          audio?.completeModelTurn?.();lastUser=lastUser.trim();lastModel=lastModel.trim();transcriptSpeaker='complete';if(transcriberConnected)transcriberReady=true;scheduleTurnRecovery();
        }
        render();
      },
    ...
    await cleanupLiveSession({clearDraft:false,hide:false});sessionState='error';render();
  }
}
async function registerDraft(){
  ...
  writeRecords([...(current.records||[]),...records]);registeredCount+=records.length;draft.clear();lastModel='';transcriptSpeaker='complete';render();
...
document.addEventListener('click',e=>{
  const button=e.target.closest('[data-live-action]');if(!button)return;
...
addEventListener('popstate',async()=>{
  if(!liveHistoryArmed&&!pendingExplicitEnd)return;
  liveHistoryArmed=false;pendingExplicitEnd=false;
  await cleanupLiveSession();
  location.reload();
});
function emergencyRelease(){
  if(recoveryTimer){clearTimeout(recoveryTimer);recoveryTimer=null}
  try{transcriber?.close()}catch{}transcriber=null;transcriberReady=false;transcriberConnected=false;
  try{transport?.close()}catch{}transport=null;
  try{audio?.stopCapture?.()}catch{}
}
addEventListener('pagehide',emergencyRelease);
addEventListener('beforeunload',emergencyRelease);
...
queuePatch();
console.info('[PFC Gemini Live]',{version:LIVE_VERSION,transcriber:TRANSCRIBE_MODEL,audioBufferMs:LIVE_AUDIO_TARGET_BUFFER_SEC*1000,freeTier:true,turnGuard:true});
