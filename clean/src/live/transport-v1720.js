import { GAS_URL, buildSetupMessage, buildOpeningMessage } from './config-v1720.js?v=1.7.20';
...
export async function issueLiveToken() {
  const response = await fetch(GAS_URL, {
    method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({taskType:'liveToken'}),redirect:'follow'
  });
  ...
  if (!token) {
    ...
    const error=new Error(String(msg));error.liveDiagnostic=diagnostic;throw error;
  }
...
export class GeminiLiveTransport {
  ...
  async connect() {
    this.onState('token');let tokenResult;
    try {tokenResult=await issueLiveToken();this.onDiagnostic({stage:'token',ok:true,...tokenResult.diagnostic});}
    catch (error) {if (error?.liveDiagnostic) this.onDiagnostic({stage:'token',ok:false,...error.liveDiagnostic});throw error;}
    const token=tokenResult.token;this.onState('connecting');
  ...
  async handleMessage(msg) {
    if (msg.toolCall?.functionCalls) {this.onTrace({type:'tool-call',detail:`count=${msg.toolCall.functionCalls.length}`});await this.onToolCall(msg.toolCall.functionCalls);}
    if (msg.toolCallCancellation?.ids) {this.onTrace({type:'tool-cancel',detail:`count=${msg.toolCallCancellation.ids.length}`});this.onToolCancellation(msg.toolCallCancellation.ids);}
    if (msg.serverContent) {
      ...
      let audioChunks=0;
      for (const part of s.modelTurn?.parts || []) if (part?.inlineData?.data) {audioChunks++;this.onAudio(part.inlineData.data,part.inlineData.mimeType || 'audio/pcm;rate=24000');}
      if (audioChunks) this.onTrace({type:'audio',detail:`chunks=${audioChunks}`});
      if (s.interrupted) {this.onTrace({type:'interrupted'});this.onInterrupted();}
      if (s.turnComplete) {this.onTrace({type:'turn-complete'});this.onState('turn-complete');}
    }
    if (msg.goAway) {this.onTrace({type:'go-away'});this.onState('go-away',msg.goAway);}
  }
  sendOpening() { this.sendObject(buildOpeningMessage()); }
  sendAudio(bytes) {if (!this.ready || !bytes?.byteLength) return;this.sendObject({realtimeInput:{audio:{data:bytesToBase64(bytes),mimeType:'audio/pcm;rate=16000'}}});}
