...
import { LIVE_VERSION } from './config-v1716.js?v=1.7.16';
import { LiveMealDraft } from './draft-v1716.js?v=1.7.16';
import { GeminiLiveTransport } from './transport-v1716.js?v=1.7.16';
...
function loadCss(){
  if(document.getElementById('pfc-live-v1716-css'))return;
  const link=document.createElement('link');
  link.id='pfc-live-v1716-css';
  link.rel='stylesheet';
  link.href=new URL('../../assets/live-v170.css?v=1.7.16',import.meta.url).href;
...
function itemStatus(item){
  ...
  if(item.unresolved)return item.candidateNames?.length?'Food Master候補を確認中':'Food Master未登録・公式情報を確認中';
...
function render(){
  ...
  sheet.innerHTML=`
    ...
    <div class="sv4-body">
      ...
      ${rawInfo?`<div class="pfc-live-diagnostic"><strong>生PCM診断</strong><span>直前AI音声 ${rawInfo.durationSec.toFixed(1)}秒 / ${rawInfo.chunkCount} chunks。①は一括基準、②は連続ストリーム検証です。</span></div>`:''}
...
function patchVersion(){
  ...
  if(runtime&&!runtime.textContent.includes(LIVE_VERSION))runtime.innerHTML=runtime.innerHTML.replace(/v1\.\d+\.\d+/,LIVE_VERSION);
}
...
async function registerDraft(){
  ...
  for(const item of items){
    ...
    if(record&&item.assumed&&item.standardLabel&&record._clean){
      record._clean.amountSource='food-master-default';
      record._clean.displayAmount=item.standardLabel;
    }
...
queuePatch();
console.info('[PFC Gemini Live]',{version:LIVE_VERSION,transcriber:TRANSCRIBE_MODEL,audioBufferMs:LIVE_AUDIO_TARGET_BUFFER_SEC*1000,searchFallback:true});
